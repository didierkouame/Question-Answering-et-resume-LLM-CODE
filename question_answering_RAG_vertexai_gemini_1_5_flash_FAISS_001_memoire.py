...
def retriever(message):
    # étape 1 Embedding de la requête utilisateur sans redéfinir la clé
    llm = VertexAI(model_name="gemini-1.5-flash-001", temperature=0.3)
    embedding_model = VertexAIEmbeddings("textembedding-gecko")
    ...
    # étape 4 générer la réponse à partir de la requête
    ...
    with open("question_answering_RAG_vertexai_FAISS_gemini_1_5_flash_001.txt", "w") as f:
        f.write(response)
